...
module Core.Evaluate(normalise, normaliseTrace, normaliseC, normaliseAll,
                ...
                lookupNames, lookupTy, lookupP, lookupDef, lookupFCDef, lookupVal,
...
data Context = MkContext { 
                  ...
                  next_tvar    :: Int,
                  definitions  :: Ctxt (FC, Def, Accessibility, Totality) 
                } deriving Show
...
ctxtAlist ctxt = map (\(n, (_, d, a, t)) -> (n, d)) $ toAlist (definitions ctxt)

veval ctxt env t = evalState (eval False ctxt [] env t []) initEval

addToCtxt :: FC -> Name -> Term -> Type -> Context -> Context
addToCtxt fc n tm ty uctxt 
    = let ctxt = definitions uctxt 
          ctxt' = addDef n (fc, Function ty tm, Public, Unchecked) ctxt in
          uctxt { definitions = ctxt' } 

setAccess :: Name -> Accessibility -> Context -> Context
setAccess n a uctxt
    = let ctxt = definitions uctxt
          ctxt' = updateDef n (\ (fc, d, _, t) -> (fc, d, a, t)) ctxt in
          uctxt { definitions = ctxt' }

setTotal :: Name -> Totality -> Context -> Context
setTotal n t uctxt
    = let ctxt = definitions uctxt
          ctxt' = updateDef n (\ (fc, d, a, _) -> (fc, d, a, t)) ctxt in
          uctxt { definitions = ctxt' }

addCtxtDef :: FC -> Name -> Def -> Context -> Context
addCtxtDef fc n d c = let ctxt = definitions c
                          ctxt' = addDef n (fc, d, Public, Unchecked) ctxt in
                          c { definitions = ctxt' }

addTyDecl :: FC -> Name -> NameType -> Type -> Context -> Context
addTyDecl fc n nt ty uctxt 
    = let ctxt = definitions uctxt
          ctxt' = addDef n (fc, TyDecl nt ty, Public, Unchecked) ctxt in
          uctxt { definitions = ctxt' }

addDatatype :: FC -> Datatype Name -> Context -> Context
addDatatype fc (Data n tag ty cons) uctxt
    = let ctxt = definitions uctxt 
          ty' = normalise uctxt [] ty
          ctxt' = addCons 0 cons (addDef n
                    (fc, TyDecl (TCon tag (arity ty')) ty, Public, Unchecked) ctxt) in
          uctxt { definitions = ctxt' }
  where
    ...
    addCons tag ((n, ty) : cons) ctxt 
        = let ty' = normalise uctxt [] ty in
              addCons (tag+1) cons (addDef n
                  (fc, TyDecl (DCon tag (arity ty')) ty, Public, Unchecked) ctxt)

-- FIXME: Too many arguments! Refactor all these Bools.
addCasedef :: FC -> Name -> CaseInfo -> Bool -> Bool -> Bool -> Bool ->
...
addCasedef fc n ci@(CaseInfo alwaysInline tcdict)
...
simplifyCasedef n uctxt
   = let ctxt = definitions uctxt
         ctxt' = case lookupCtxt n ctxt of
              [(_, CaseOp ci ty [] ps _, acc, tot)] ->
                 ctxt -- nothing to simplify (or already done...)
              [(fc, CaseOp ci ty ps_in ps cd, acc, tot)] ->
                 let ps_in' = map simpl ps_in
                     pdef = map debind ps_in' in
                     case simpleCase False True False CompileTime fc pdef of
...
addOperator :: FC -> Name -> Type -> Int -> ([Value] -> Maybe Value) -> 
               Context -> Context
addOperator fc n ty a op uctxt
    = let ctxt = definitions uctxt 
          ctxt' = addDef n (fc, Operator ty a op, Public, Unchecked) ctxt in
          uctxt { definitions = ctxt' }
...
lookupP n ctxt
   = do def <-  lookupCtxt n (definitions ctxt)
        p <- case def of
          (_, Function ty tm, a, _) -> return (P Ref n ty, a)
          (_, TyDecl nt ty, a, _) -> return (P nt n ty, a)
          (_, CaseOp _ ty _ _ _, a, _) -> return (P Ref n ty, a)
          (_, Operator ty _ _, a, _) -> return (P Ref n ty, a)
        case snd p of
            Hidden -> []
            _ -> return (fst p)

lookupDef :: Name -> Context -> [Def]
lookupDef n ctxt = map qsnd $ lookupCtxt n (definitions ctxt)

lookupFCDef :: Name -> Context -> [(FC, Def)]
lookupFCDef n ctxt = map (\(fc, d, _, _) -> (fc, d)) $ lookupCtxt n (definitions ctxt)
...
lookupDefAcc n mkpublic ctxt
  ...
  where mkp (_, d, a, _) = if mkpublic && (not (n == UN "io_bind" || n == UN "io_return"))
