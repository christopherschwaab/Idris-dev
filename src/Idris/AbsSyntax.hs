...
addDeferred' :: NameType -> [(FC, Name, Type)] -> Idris ()
addDeferred' nt ns
  = do mapM_ (\(fc, n, t) -> updateContext (addTyDecl fc n nt (tidyNames [] t))) ns
       i <- getIState
       putIState $ i { idris_metavars = map (\(_, n, _) -> n) ns ++ idris_metavars i }
